...
#if DISABLED(SPINDLE_LASER_ENABLE) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(ULTRA_LCD) && DISABLED(NEWPANEL))  // try to use IO Header
...
// LCD / Controller
//
#if DISABLED(ULTRA_LCD) && DISABLED(NEWPANEL)
...
//
  ...
  #if !MB(AZTEEG_X1) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(ULTRA_LCD) && DISABLED(NEWPANEL))  // try to use IO Header
