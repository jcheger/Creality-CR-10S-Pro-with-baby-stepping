...
bool record_status[2] = {false,false};	// when working in auto level, record auto level status
...
int RTSSHOW::RTS_CheckFilement(int mode)
{
	...
	waitway = 4;
	for(Checkfilenum= 0; 0==READ(CHECK_MATWEIAL) && Checkfilenum < 50;Checkfilenum++)// no filements check
		delay(15);
...
void RTSSHOW::RTS_Init()
{
	...
	last_zoffset = probe_zoffset;
	RTS_SndData(probe_zoffset*100, 0x1026); 
...
*/
	...
	case Zoffset:
		if(recdat.data[0]>= 32768)
			probe_zoffset = ((float)recdat.data[0]-65536)/100;
		else
			probe_zoffset = ((float)recdat.data[0])/100;
		break;
	...
	case Setting:
		...
		else if(recdat.data[0] == 1)	//Bed Autoleveling
		{
			...
			settings.reset();
			record_status[0] = AutoLevelStatus;
			record_status[1] = AutoLevelStatus = true;
			...
			enqueue_and_echo_commands_P((PSTR("G1 Z0.2 F150"))); 
			if(LanguageRecbuf != 0)
	...
	case ReturnBack:
		 if(recdat.data[0] == 1)	 // return to the tool page
		 {
			...
			if(record_status[1])
			{
			 	record_status[1] = InforShowStatus = false;
				AutoLevelStatus = record_status[0];
				settings.load();
			}
	...
	case Bedlevel:
		...
		{
			if(record_status[0])	//turn on the Autolevel
			{
				RTS_SndData(3, AutoLevelIcon);	
				record_status[0] = false;
				settings.load();
			}
			else//turn off the Autolevel
			{
				RTS_SndData(2, AutoLevelIcon);
				record_status[0] = true;
				settings.reset();
			}
			last_zoffset = probe_zoffset;
			RTS_SndData(probe_zoffset*100, 0x1026); 
			eeprom_write_byte((unsigned char*)FONT_EEPROM+2, record_status[0]);
		}
...
{		
	...
	if(ms > next_rts_update_ms && InforShowStatus)
	{
		...
		{
			if (TPShowStatus && previous_cmd_ms != 0)		//need to optimize
			{
				...
				if(card.sdprinting && last_cardpercentValue != card.percentDone())
				{
				  ...
				  if(probe_zoffset != last_zoffset)	// save z offset
				  {
				  	settings.save();
					last_zoffset = probe_zoffset;
				  } 
				
				if(PrintModeChange && card.sdprinting && current_position[Z_AXIS] > 2.0 && (elapsed.value>5*60+PrintModeTime) )	// power saving mode
				{
					if(PrintMode)
					{
						tempbed = thermalManager.target_temperature_bed;
